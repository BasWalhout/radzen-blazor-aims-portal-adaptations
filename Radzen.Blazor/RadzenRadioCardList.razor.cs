...
namespace Radzen.Blazor
{
    ...
    /// </code>
    /// </example>
    public partial class RadzenRadioCardList<TValue> : FormComponent<TValue>
    {
        string ItemClass(RadzenRadioCardListItem<TValue> item) => ClassList.Create("rz-radiobutton-box")
                                                                            .Add("rz-state-active", IsSelected(item))
                                                                            .Add("rz-state-focused", IsFocused(item) && focused)
                                                                            .AddDisabled(Disabled || item.Disabled)
                                                                            .ToString();

        string IconClass(RadzenRadioCardListItem<TValue> item) => ClassList.Create("rz-radiobutton-icon")
        ...
        List<RadzenRadioCardListItem<TValue>> allItems;

        void UpdateAllItems()
        {
            allItems = items.Concat((Data != null ? Data.Cast<object>() : Enumerable.Empty<object>()).Select(i =>
            {
                var item = new RadzenRadioCardListItem<TValue>();
        ...
        List<RadzenRadioCardListItem<TValue>> items = new List<RadzenRadioCardListItem<TValue>>();

        /// <summary>
        /// Adds the item.
        /// </summary>
        /// <param name="item">The item.</param>
        public void AddItem(RadzenRadioCardListItem<TValue> item)
        {
        ...
        /// </summary>
        /// <param name="item">The item.</param>
        public void RemoveItem(RadzenRadioCardListItem<TValue> item)
        {
        ...
        protected bool IsSelected(RadzenRadioCardListItem<TValue> item)
        {
        ...
        /// </summary>
        /// <param name="item">The item.</param>
        protected async System.Threading.Tasks.Task SelectItem(RadzenRadioCardListItem<TValue> item)
        {
        ...
        bool HasInvisibleBefore(RadzenRadioCardListItem<TValue> item)
        {
            return allItems.Take(allItems.IndexOf(item)).Any(t => !t.Visible && !t.Disabled);
        }

        bool IsFocused(RadzenRadioCardListItem<TValue> item)
        {
